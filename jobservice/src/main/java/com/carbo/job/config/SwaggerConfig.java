...
public class SwaggerConfig {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(apiInfo())
                .servers(List.of(
                        new Server().url(baseUrl+"job/") // external route
                ))
    ...
    public GroupedOpenApi jobServiceApi() {
        return GroupedOpenApi.builder()
                .group("job-service-api")
                .addOpenApiCustomizer(openApi ->
                        openApi.getPaths().entrySet().removeIf(e -> !e.getKey().startsWith("/v1/jobs/external")))
                .packagesToScan("com.carbo.job.controllers.external")
                .build();
    }
